...
def csvGenerator(data):
	...
	for linha in data:
		for elemento in linha:
			if str(elemento).find("(34)") == -1 and str(elemento).find("Docente") == -1 and str(elemento).isdigit() == False: # Get rid of phone numbers and unnecessary data (Uncomment for FAMEV)
				if cont == 0:
					file.write('"' + elemento + '"' + ',')
				elif cont == 3:
					file.write('"' + elemento + '"')
